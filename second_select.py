...
def get_one_csv_indicators(dir_name):
    ...
    csv_file_name = 'one_indicators_test.csv'
    if len(indicator_array) > 0:
        ind_pd_new = pd.DataFrame(indicator_array, columns=indicator_cols)
        ind_pd_new.to_csv('%s/%s' % ('test2', csv_file_name), index=False)
...
def test_unit1(dir_name):
    get_one_csv_indicators(dir_name)


test_path1 = '/opt/wfo_data_csv/youxian_canshu/rb_zs_TB_WOBV_DX_QG_GZ_ADC_DK_20180323-102633_grid.yaml'
# test_unit1('/opt/wfo_data_csv/WH_WOBV_DX_QG_GZ_all_20180102/j_zs_WH_WOBV_DX_QG_GZ_20180102-81542_grid.yaml')
test_unit1(test_path1)
