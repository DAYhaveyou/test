...
def select_csv_from_setting_yaml_return_value_database(yaml_file, file_name):
    f = open(yaml_file)
    raw_value0 = yaml.load(f)
    ...
    # combination
    print len(storage_values)
    print type(storage_values)
    if len(storage_values) >= 0:
        return
    if flag == 1:
        for i in range(1, len(temp_data)):
            for j in temp_data[i]:
                for k in storage_values:
                    if j[0] == k[0]:
                        continue
                    else:
                        storage_values.append(j)
...
select_csv_from_setting_yaml_return_value_database('test_make_setting.yaml', path)
